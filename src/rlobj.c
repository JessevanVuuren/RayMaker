...
char *ReadObjectName(GenericFile *file) {
    ClearWhitespace(file);
    ...
    int i;
    for (i = 0; !isspace(file->data[i + 2]) && file->data[i + 2] != '\0'; i++) {
        name[i] = file->data[i + 2];
    }
    return name;
}
...
OBJMesh LoadObjMesh(OBJFile *file) {
    ...
    while (*file->data.data) {
        if (*file->data.data == 'v') {
            ...
            if (seen_o) break;
            mesh_name = ReadObjectName(&file->data);
            seen_o = true;
    ...
    return (OBJMesh) {.mesh = m, .mat_hash = mat_hash, .name = mesh_name};
}
